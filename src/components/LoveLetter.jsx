...
export default function LoveLetter({ onNext }) {
  ...
  const steps = [
    ...
    },
    {
      ...
      content: (
        <div className="bg-pink-50/80 p-6 rounded-2xl border border-pink-200 text-pink-950 italic text-xl font-medium leading-loose text-center my-4 shadow-inner">
    ...
    },
    {
      ...
      content: (
        <div className="space-y-4 my-2">
          {/* Eyes Photo Frame */}
          <div className="w-full h-44 rounded-2xl overflow-hidden shadow-lg border-2 border-pink-300 relative">
            <img 
          ...
          <div className="bg-pink-50/80 p-5 rounded-2xl border border-pink-200 text-pink-950 italic text-lg font-medium leading-relaxed text-center shadow-inner">
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen p-4 md:p-6 font-serif">
      {!opened ? (
        <motion.div
          ...
          className="cursor-pointer bg-gradient-to-br from-amber-50 via-pink-50 to-orange-100 border-2 border-pink-200 p-10 md:p-12 rounded-3xl shadow-2xl text-center max-w-sm relative"
        >
          <div className="text-7xl mb-4 animate-bounce">💌</div>
          <h3 className="text-2xl font-bold text-pink-600">For Zahin</h3>
          <p className="text-pink-400 text-sm mt-2 italic">Tap seal to open your special letter</p>
        </motion.div>
      ) : (
        <motion.div
          initial={{ scale: 0.9, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          className="bg-[#fffdfa] border border-pink-200/80 p-6 md:p-8 rounded-3xl shadow-2xl max-w-xl w-full relative font-serif text-amber-950"
        >
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {steps[step].title && (
                <h2 className="text-2xl md:text-3xl font-bold mb-4 text-pink-600 italic">
                  {steps[step].title}
                </h2>
              )}
              {steps[step].subtitle && (
                <p className="text-lg font-medium text-pink-500 italic mb-2">
                  {steps[step].subtitle}
                </p>
              )}
              {steps[step].content}
          ...
          <div className="mt-6 pt-4 border-t border-pink-100 flex justify-between items-center">
            {step > 0 ? (
              <button
                ...
                className="px-4 py-2 bg-pink-100 text-pink-600 rounded-full hover:bg-pink-200 transition-all text-xs font-medium"
              >
                ← Previous
              </button>
            ) : <div />}
            ...
            <button
              ...
              className="px-6 py-2.5 bg-gradient-to-r from-pink-500 to-rose-500 text-white rounded-full hover:from-pink-600 hover:to-rose-600 transition-all text-sm font-semibold shadow-md shadow-pink-300/50"
            >
