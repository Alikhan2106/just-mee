...
export default function LoveLetter({ onNext }) {
    ...
    },
    {
      ...
      content: (
        <div className="text-center space-y-3 my-2">
          ...
          <p className="text-xs sm:text-lg text-gray-800 leading-relaxed italic">
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[calc(100dvh-5rem)] p-3 sm:p-6 font-serif">
      {!opened ? (
        <motion.div
          ...
          whileTap={{ scale: 0.98 }}
          className="cursor-pointer bg-gradient-to-br from-amber-50 via-pink-50 to-orange-100 border-2 border-pink-200 p-8 sm:p-12 rounded-3xl shadow-xl text-center max-w-xs sm:max-w-sm w-full"
        >
          ...
          <p className="text-pink-400 text-xs sm:text-sm mt-1.5 italic">Tap seal to open your special letter</p>
        </motion.div>
      ) : (
        <motion.div
          initial={{ scale: 0.95, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          className="bg-[#fffdfa] border border-pink-200 p-4 sm:p-7 rounded-3xl shadow-xl max-w-lg sm:max-w-xl w-full font-serif text-amber-950 flex flex-col max-h-[82dvh]"
        >
          <div className="flex justify-between items-center mb-2.5 border-b border-pink-100 pb-2">
          ...
          <div className="overflow-y-auto pr-1 max-h-[62dvh] my-auto">
            <AnimatePresence mode="wait">
              ...
              >
                {steps[step].title && (
                  <h2 className="text-lg sm:text-2xl font-bold mb-2 text-pink-600 italic">
                    {steps[step].title}
                  </h2>
                )}
                {steps[step].subtitle && (
                  <p className="text-sm sm:text-lg font-medium text-pink-500 italic mb-1.5">
                    {steps[step].subtitle}
                  </p>
                )}
                {steps[step].content}
          ...
          <div className="mt-4 pt-3 border-t border-pink-100 flex justify-between items-center gap-2">
            {step > 0 ? (
              <button
                onClick={() => setStep((prev) => prev - 1)}
                className="px-3 py-2 bg-pink-100 text-pink-600 rounded-full hover:bg-pink-200 transition-all text-xs font-medium min-h-[40px] min-w-[70px]"
              >
                ← Back
              </button>
            ) : <div />}
            ...
            <button
              ...
              className="px-4 sm:px-6 py-2 bg-gradient-to-r from-pink-500 to-rose-500 text-white rounded-full hover:from-pink-600 hover:to-rose-600 transition-all text-xs sm:text-sm font-semibold shadow-md shadow-pink-300/50 min-h-[40px]"
            >
